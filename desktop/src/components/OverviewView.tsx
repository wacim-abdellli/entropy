...
import { AlertTriangle, ArrowUpRight, CircleDot, FolderPlus, GitBranch, HardDrive, Play, RefreshCw, Terminal } from 'lucide-react';
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto bg-[var(--color-surface-0)] animate-enter">
      <header className="sticky top-0 z-10 h-14 px-7 flex items-center justify-between border-b border-[var(--color-border-subtle)] bg-[var(--color-surface-0)]/95 backdrop-blur">
        ...
        <div className="flex items-center gap-2"><button type="button" onClick={onInspectFolder} className="h-8 px-2.5 rounded-md text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-surface-2)] flex items-center gap-1.5"><FolderPlus className="w-3.5 h-3.5" />Add folder</button><button type="button" title="Refresh workspaces" onClick={onRefresh} disabled={isLoading} className="w-8 h-8 rounded-md hover:bg-[var(--color-surface-2)] text-[var(--color-text-secondary)] flex items-center justify-center disabled:opacity-50"><RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} /></button></div>
      </header>
